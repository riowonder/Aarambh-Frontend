...
export default function UserForgotPswdOTP() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/auth/user-reset-password-otp`, {
        email,
        otp 
      });
      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-xl shadow-lg p-8 flex flex-col gap-6">
        <div className="flex flex-col gap-2 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            We have sent a verification code to your user email address
          </p>
        </div>
