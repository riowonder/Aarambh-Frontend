...
export default function UserResetPswd() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/auth/user-change-password`, {
        email: state.email,
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-muted overflow-hidden fixed top-0 left-0">
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-xl shadow-lg p-8 flex flex-col gap-6">
        <div className="flex flex-col gap-2 text-center">
          <h2 className="text-2xl font-bold tracking-tight">Set User Password</h2>
          <p className="text-sm text-muted-foreground">Enter your new user password below</p>
        </div>
