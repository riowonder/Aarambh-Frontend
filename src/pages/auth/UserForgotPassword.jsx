...
export default function UserForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/auth/user-reset-password`, {
        email: email
      });
      toast.success("Reset code sent successfully!");
      navigate("/user-forgot-password/otp-verification", { state: { email } });
    } catch (error) {
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-muted overflow-hidden fixed top-0 left-0">
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-xl shadow-lg p-8 flex flex-col gap-6">
        <div className="flex flex-col gap-2 text-center">
          <h2 className="text-2xl font-bold tracking-tight">Reset user password</h2>
        ...
        <div className="flex flex-col gap-2 text-center text-sm">
          <p className="text-gray-500">
            Remember your password? <NavLink to="/user-login" className="underline hover:text-black">Sign In</NavLink>
          </p>
          <p className="text-gray-500">
